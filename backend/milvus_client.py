...
class MilvusManager:
    ...
    def init_collection(self, dense_dim: int = 1024):
        ...
        注意：阿里云百炼 embedding 模型最大支持 1024 维
        :param dense_dim: 密集向量维度，默认 1024
        """
        ...
        if client.has_collection(self.collection_name):
            try:
                ...
                fields = collection_info.get("schema", {}).get("fields", []) if "schema" in collection_info else collection_info.get("fields", [])
                for field in fields:
                    if field.get("name") == "dense_embedding":
                        existing_dim = field.get("dimension", 0)
                        if existing_dim > 0 and existing_dim != dense_dim:
                            print(f"[MilvusManager] 现有集合维度 {existing_dim} 与目标维度 {dense_dim} 不匹配，正在重建...")
                            client.drop_collection(self.collection_name)
                            needs_rebuild = True
                        elif existing_dim == dense_dim:
                            print(f"[MilvusManager] 集合已存在，维度 {dense_dim} 匹配，无需重建。")
                        break
                else:
                    needs_rebuild = True
            except Exception as e:
                print(f"[MilvusManager] 检查集合维度失败，将重建: {e}")
                client.drop_collection(self.collection_name)
                needs_rebuild = True
        else:
            needs_rebuild = True
        ...
        print(f"[MilvusManager] 创建新集合，维度: {dense_dim}")
